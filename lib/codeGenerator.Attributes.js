...
var codeGeneratorBase = require('./codeGenerator.Base');
...
module.exports = {
  ...
  concatenateAttributeNodes: function (nodes, code_generation_context) {
    ...
    var self = this;
    var is_setting_value = code_generation_context.isSettingValue;
  ...
  parseAttributes: function (code_generation_context) {
    ...
    var result = '';
    var original_ast_node = code_generation_context.astNode;
    var adapter = code_generation_context.adapter;
    var attributes_ast = code_generation_context.astNode.attributes;

    if (attributes_ast.length) {
      result = adapter.accumulateFinalAttributesResult(code_generation_context, this.concatenateAttributeNodes(attributes_ast, code_generation_context));
    } else {
      result = adapter.accumulateFinalAttributesResult(code_generation_context, '');
    }
...
/// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  keyValue: function (code_generation_context) {
    ...
    var adapter = code_generation_context.adapter;
    var key_property_definition = adapter.defineAttribute(code_generation_context, code_generation_context.objectName, '"' + key_name + '"');
    var key_property_setting = this.concatenateAttributeNodes(code_generation_context.astNode.value, code_generation_context);

    return adapter.setAttributeValue(code_generation_context, key_property_definition, key_property_setting);
  },
  ...
  singleValue: function (code_generation_context) {
    ...
    if (code_generation_context.isSettingValue) {
      return adapter.accumulateAttributeValue(code_generation_context, code_generation_context.objectName, '"' + code_generation_context.astNode.value + '"');
    } else {
      // Single properties like checked or disable are parsed as singleValue
      var key_property_definition = adapter.defineAttribute(code_generation_context, code_generation_context.objectName, '"' + key_name + '"');
      return adapter.setAttributeValue(code_generation_context, key_property_definition, '');
    }
  },
  ...
  singleKey: function (code_generation_context) {
    ...
    if (code_generation_context.isSettingValue) {
      ...
      var key_property_definition = adapter.defineAttribute(code_generation_context, code_generation_context.objectName, '"' + code_generation_context.astNode.value + '"');
      return adapter.setAttributeValue(code_generation_context, key_property_definition, '');
    }
    return '';
  },
...
/// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  handlebarsATTRSINGLEEVALUATION: function (code_generation_context) {
    ...
    if (code_generation_context.isSettingValue) {
      return adapter.accumulateAttributeValue(code_generation_context, code_generation_context.objectName, expression);
    } else {
      var key_property_definition = adapter.defineAttribute(code_generation_context, code_generation_context.objectName, expression);
      return adapter.setAttributeValue(code_generation_context, key_property_definition, '');
    }
  },
  ...
  handlebarsATTRIF: function (code_generation_context) {
    ...
    var result = 'if (' + this._handlebarsEvaluateCondition(code_generation_context, ast_node.condition) + ') {';
  ...
  handlebarsATTREACH: function (code_generation_context) {
    ...
    var ast_node = code_generation_context.astNode;
    var iterator_variable_name = this._generateUniqueId('eachIterator');
    var iterator_index = iterator_variable_name + 'Index';
    var each_context_name = this._generateUniqueId('contextName');
    var iterator_name = this._handlebarsEvaluateCondition(code_generation_context, ast_node.iterator);
    var result = '';
    var each_context = {
